...
void AVL::populateAVLTree(AVL& tree) {
	int sinArray[NUM];
	for (int i = 0; i < NUM; ++i) sinArray[i] = 100000000 + i;
	std::random_shuffle(std::begin(sinArray), std::end(sinArray));
	...
	if (!outfile.is_open()) {
		std::cerr << "Failed to open inorder.txt for writing." << std::endl;
		return;
	}
	std::cout << "INORDER (NUM=" << NUM << ", height=" << tree.GetRoot()->height << "): root node has sin:" << tree.GetRoot()->empl.sin << std::endl << std::endl;
	outfile << "INORDER (NUM=" << NUM << ", height=" << tree.GetRoot()->height << "): root node has sin:" << tree.GetRoot()->empl.sin << std::endl << std::endl;
	tree.display();
	outfile.close();
	std::cout << std::endl << "DONE: Tree Has been populated." << std::endl << std::endl;
}
